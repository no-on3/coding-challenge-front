        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="flex items-center justify-between h-16">
              <div class="flex-shrink-0">
                <a href="/" class="uppercase font-bold text-white tracking-widest text-lg">Coding Challenge Store</a>
              </div>
              <div class="md:block">
                <div class="ml-10 flex items-baseline">
                  <div class="inline-block relative w-64">
                    <select v-model="category" class="bg-yellow-400 block appearance-none
                     text-gray-800 w-full border border-yellow-400
                      hover:border-yellow-300 px-4 py-2 pr-8
                       rounded shadow leading-tight focus:outline-none
                        focus:shadow-outline text-sm" @change="submitSearch({category : $event.target.value})">
                      <option v-for="category in categoryList" :key="category" selected>{{category}}</option>
                    </select>
        ...
        <header>
          <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5w-full">
            <nav aria-label="product-filter" class="border-b-2 border-indigo-100">
              <div class="flex items-center justify-between h-16">
                <div class="flex-shrink-0">
                  <price-range :default-min="0" :default-max="1000" @set-filter="submitSearch({price : $event})" />
                </div>
                <div>
              ...
              class="bg-gray-100 appearance-none border-2 border-gray-100 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500"
                       aria-label="Full name" @keyup="submitSearch({searchQuery : $event.target.value})">
                    </div>
                  </form>
                </div>
              </div>
